  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex w-1/2 bg-indigo-600 p-12 flex-col justify-between relative overflow-hidden">
        <div className="z-10">
          <div className="bg-white/20 backdrop-blur-md w-12 h-12 rounded-xl flex items-center justify-center mb-8 text-white">
            <UserIcon size={24} />
          </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 bg-white">
        <div className="max-w-md w-full">
          ...
          {error && (
            <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 font-bold text-xs animate-in fade-in slide-in-from-top-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
            {!isLogin && !showAdminMode && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="Ali Khan" className="w-full pl-12 pr-4 py-3.5 md:py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-0 focus:border-indigo-600 outline-none transition-all text-sm" />
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder={showAdminMode ? "admin@bizgen.ai" : "ali@gmail.com"} className="w-full pl-12 pr-4 py-3.5 md:py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-0 focus:border-indigo-600 outline-none transition-all text-sm" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="w-full pl-12 pr-4 py-3.5 md:py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-0 focus:border-indigo-600 outline-none transition-all text-sm" />
              </div>
            </div>
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-indigo-600 text-white py-4 md:py-5 rounded-2xl font-black text-lg md:text-xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group disabled:opacity-50"
            >
              {isLoading ? (
          ...
          {!showAdminMode && (
            <p className="mt-8 md:mt-10 text-center text-slate-500 font-medium text-sm">
          ...
          {showAdminMode && (
            <div className="mt-8 p-5 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-3">
              ...
              <p className="text-[10px] md:text-xs text-amber-700 leading-relaxed font-bold">
